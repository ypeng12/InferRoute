...
from inferroute.observability import ROUTING_DECISION_TOTAL
...
class Router:
    async def get_backend_stats(self, backend: str) -> dict[str, float]:
        ...
        try:
            ...
            ttft_raw = await client.get(f"inferroute:stats:{backend}:ttft")
            p95_raw = await client.get(f"inferroute:stats:{backend}:p95")
    ...
    async def record_metrics(self, backend: str, ttft_ms: float, latency_ms: float, success: bool) -> None:
        ...
        try:
            ...
            # Update TTFT
            old_ttft = await client.get(f"inferroute:stats:{backend}:ttft")
            new_ttft = ttft_ms if not old_ttft else (alpha * ttft_ms + (1 - alpha) * float(old_ttft))
            await client.set(f"inferroute:stats:{backend}:ttft", str(new_ttft))
            
            # Update P95
            old_p95 = await client.get(f"inferroute:stats:{backend}:p95")
            new_p95 = latency_ms if not old_p95 else (alpha * latency_ms + (1 - alpha) * float(old_p95))
            await client.set(f"inferroute:stats:{backend}:p95", str(new_p95))
