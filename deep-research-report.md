# InferRoute 生产级低延迟 LLM 推理路由与可观测平台设计报告
...
InferRoute 的目标用户不应泛泛定义为“所有 LLM 开发者”，而应明确到三类：
...
InferRoute 的推荐高层架构如下。其核心原则是：**控制面与数据面分离，在线请求路径尽量短，观测与成本计算异步化，回退路径内建而不是事后补救。**
...
| 指标名 | 类型 | 说明 |
|---|---|---|
| `inferroute_request_total{tenant,model,backend,status}` | Counter | 总请求数 |
| `inferroute_request_latency_seconds` | Histogram | 端到端时延 |
| `inferroute_ttft_seconds` | Histogram | 首 token 时间 |
| `inferroute_inter_token_latency_seconds` | Histogram | token 间延迟 |
| `inferroute_queue_depth{backend}` | Gauge | 调度队列深度 |
| `inferroute_cache_hit_total{type=exact|prefix|semantic}` | Counter | 各类缓存命中 |
| `inferroute_provider_cost_usd_total{backend,tenant}` | Counter | 累积成本 |
| `inferroute_validation_fail_total{reason}` | Counter | 输出校验失败数 |
| `inferroute_routing_decision_total{policy,backend}` | Counter | 路由落点统计 |
| `inferroute_fallback_total{from,to,reason}` | Counter | 回退事件 |
| `inferroute_rate_limited_total{scope}` | Counter | 限流触发数 |
...
COPY . /app
EXPOSE 8080
CMD ["uvicorn", "inferroute.main:app", "--host", "0.0.0.0", "--port", "8080"]
```
...
kind: Deployment
metadata:
  name: inferroute-gateway
spec:
  replicas: 2
  selector:
    matchLabels:
      app: inferroute-gateway
  template:
    metadata:
      labels:
        app: inferroute-gateway
    spec:
      containers:
        - name: gateway
          image: ghcr.io/you/inferroute:latest
          ports:
            - containerPort: 8080
          env:
            - name: REDIS_URL
